...
// ============================================================
//  KYC — FACE MATCHING via Face++
//  POST /kyc/compare
//  Body: { idImageBase64, selfieBase64, userId }
// ============================================================
app.post('/kyc/compare', async (req, res) => {
  const { idImageBase64, selfieBase64, userId, idPhotoUrl, selfieUrl, docType } = req.body;
  ...
  try {
    ...
    // ── THRESHOLDS (owner requirement) ──
    // ≥ 80  → auto-verified (no admin action needed)
    // 65–79 → manual_review (admin sees in KYC queue)
    // < 65  → rejected automatically
    let kycFinalStatus;
    if      (confidence >= 80) kycFinalStatus = 'verified';
    else if (confidence >= 65) kycFinalStatus = 'manual_review';
    else                       kycFinalStatus = 'rejected';

    const matched = confidence >= 65; // anything ≥65 is a "match" (65-79 needs admin)
    const threshold = 65;
    ...
    await db.collection('kyc_submissions').doc(userId).set({
      ...
      idPhotoUrl:  idPhotoUrl  || null,   // Cloudinary URL — shown in admin KYC panel
      selfieUrl:   selfieUrl   || null,   // Cloudinary URL — shown in admin KYC panel
      docType:     docType     || 'id',
